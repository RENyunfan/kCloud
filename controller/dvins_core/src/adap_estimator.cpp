...
Eigen::Vector3d esti_position(0.131932654922,0.0,0.827372696932);
Eigen::Vector3d esti_position_1(0.131932654922,0.0,0.827372696932);
...
//初始化读取img_msg 初始化值
void imgcallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr &Img_msg)
{
  ...
  if(enter)
  {
     for(int32_t i=0; i<Img_msg->detections.size();i++)
    {
      
      const int32_t ID = Img_msg->detections[i].id[0];       
      // ROS_INFO("ID:[%d]",ID);
      
      // ROS_INFO("position[%f,%f]",tag_image_center.x,tag_image_center.y); 
      // 读取相机坐标下的坐标
      tag_pose.position.x=Img_msg->detections[i].pose.pose.pose.position.x;
      tag_pose.position.y=Img_msg->detections[i].pose.pose.pose.position.y;
      tag_pose.position.z=Img_msg->detections[i].pose.pose.pose.position.z;    
...
void TagPoseBufcallback(const apriltag_ros::AprilTagDetectionArray::ConstPtr &Img_msg )
{ 
...
void visual_velocity(pair<double,map<int,geometry_msgs::Pose2D> > &tag_center_t_lib_0,pair<double,map<int,geometry_msgs::Pose2D> > &tag_center_t_lib_1)
{
   ...
   while(map_it_center_1!=tag_center_t_lib_1.second.end())
  {
      ...
      ROS_INFO("ID:[%d],tag_visual_velocity[%f,%f]",map_it_center_1->first,center_velocity_buf.x,center_velocity_buf.y);
...
void callback(const sensor_msgs::Imu::ConstPtr &Imu_msg, const apriltag_ros::AprilTagDetectionArray::ConstPtr &Img_msg)
{
...
int main(int argc, char** argv) {
   ros::init(argc, argv, "adap_estimator");
   ros::NodeHandle n("~");
   ...
   esti_pub = n.advertise<nav_msgs::Odometry>("/odometry_pub", 1);
   ros::Subscriber sub_image=n.subscribe("/tag_detections",10,imgcallback);
   ROS_INFO_STREAM( " Hello , ROS! " ) ;
   
   //初始的特征点集初始化tag_pose,tag_image_center  
   message_filters::Subscriber<sensor_msgs::Imu> imu_sub(n, "/firefly/imu", 1);
   message_filters::Subscriber<apriltag_ros::AprilTagDetectionArray> img_sub(n, "/tag_detections", 1);
   typedef sync_policies::ApproximateTime<sensor_msgs::Imu, apriltag_ros::AprilTagDetectionArray> MySyncPolicy;
