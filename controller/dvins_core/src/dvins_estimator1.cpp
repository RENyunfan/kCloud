...
void update_estimate(map<int,geometry_msgs::Pose> &tag_pose_lib,map<int,A_COFF> &ID_Coff,Vector3d &esti_p,Vector3d &esti_v)
{
   ...
   //  delt_x=real_R * real_velocity;
    delt_x=real_R * real_velocity - Co_1 - Co_2;
   //  std::cout << "this is deltx"<<delt_x << std::endl;
   //  delt_x=esti_v-Co_1-Co_2;
   // 这里的期望位置是设定的，而不是读取的期望值
    esti_p +=(delt_x*Delta_t_estimate);
    std::cout << "this is esti_p"<< Co_2 <<std::endl<<delt_x*Delta_t_estimate << std::endl;
...
int main(int argc, char** argv) {
   ...
   // message_filters::Subscriber<dvins_core::ArucoDetectionInImageArray> img_sub(n, "/aruco_detector/aruco_detection", 1);
